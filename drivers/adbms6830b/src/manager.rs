...
mod lines {
    ...
    impl Lines {
        ...
        pub(in crate::segments) async fn read<G: ReadableGroup>(&mut self, line: LineId) -> Result<adbms6830b::spi::Responses<G>, Error<SpiError>> {
...
mod chips {
    ...
    impl Chips {
        ...
        pub(in crate::segments) fn init() -> Self {
            Self {
                chips: [ChipData::new(); ChipId::VARIANT_COUNT]
            }
        }
    ...
    pub(in crate::segments) struct ChipData {
        /// Expected command count for a chip. This gets incremented
        /// for each chip whenever we send a command where INC is true.
        pub(in crate::segments) ccnt_expected: u8,

        /// Reported command count for a chip. This is stored for each chip on reads.
        pub(in crate::segments) ccnt_reported: u8,
    }
    impl ChipData {
        /// ChipData with default values.
        const fn new() -> Self {
            Self { ccnt_expected: 0, ccnt_reported: 0 }
        }
    }
}
...
pub enum ResponseError {
    ...
    PecFailed,
    /// When processing a read response, this chip's index was out of bounds. In other words, this chip's LineIndex was not included
    /// in the `Line`'s response.
    /// 
    /// This means that a read to one of the `Line`s returned data for less chips than expected. For example, we might currently have 8 chips assigned to
    /// Line A, but a read to Line A only returned 6 chips (meaning that when we go to lookup Chip7 or Chip8 on the read response, we get an out of bounds error).
    /// This likely indicates that there is a bug in how we are tracking the Chip->Line assignments.
    OutOfBounds,
}
impl ResponseError {
...
pub struct Responses<G> {
    chips: [Result<G, ResponseError>; ChipId::VARIANT_COUNT],
}
...
impl<G: ReadableGroup> Responses<G> {
    ...
    pub fn device(&self, chip: ChipId) -> Result<G, ResponseError> { self.chips[chip as usize] }
    ...
    pub fn iter(&self) -> impl Iterator<Item = (ChipId, Result<G, ResponseError>)> + '_ {
    ...
    pub async fn read<G: ReadableGroup>(&mut self) -> Responses<G> {
        ...
        Responses {
            chips: ChipId::LIST.map(|chip_id| {
                let line_index = self.lines.split().lineindex_from_chipid(chip_id);

                let line_responses = match line_index.line() {
                    LineId::LineA => &line_a,
                    LineId::LineB => &line_b,
                };

                match line_responses {
                    Ok(line_responses) => match line_responses.get(line_index.index()) {
                        Some(chip_response) => {
                            match chip_response.pec() {
                                adbms6830b::spi::PecStatus::Success => {
                                    // update command counter on successful pec
                                    self.chips.get_mut(chip_id).ccnt_reported = chip_response.command_counter();
                                    Ok(chip_response.data())
                                },
                                adbms6830b::spi::PecStatus::Failed => {
                                    // the pec wasnt successful so cant reliably update the command counter
                                    // also no data for you
                                    Err(ResponseError::PecFailed)
                                }
                            }
                        },
                        None => Err(ResponseError::OutOfBounds),
                    },
                    Err(err) => Err(ResponseError::LineFailed(*err, line_index.line())),
                }
            }),
        }
    }
