//! This is a "PAC" for the ADBMS6830B (it isn't really a PAC since ADBMS6830B isn't an on-chip peripheral, but it's close enough). Basically, this crate provides the types that map to the
//! chip's registers and commands, basically taken straight from the datasheet and encoded into Rust. The full ADBMS6830B driver can use this interface to interact with registers
//! and such without doing any manual bit fiddling.

// This is being used since the [b7:b0] syntax for describing bit fields (used a lot in this module) is interpreted as broken intradoc
// links by rustdoc 
#![allow(rustdoc::broken_intra_doc_links)]

#[rustfmt::skip]
pub mod commands;
#[rustfmt::skip]
pub mod pec;
#[rustfmt::skip]
pub mod registers;